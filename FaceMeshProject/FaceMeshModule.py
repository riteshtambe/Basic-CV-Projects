...
class FaceMeshDetector:
    def __init__(self,staticMode=False,maxFaces=3,refineLms=False,minDetectionCon=0.5,minTrackCon=0.5):
...
def main():
    ...
    while True:
        ...
        img = cv2.resize(img,(820,800))
        
        img,faces = detector.findFaceMesh(img)

        if len(faces)!=0:
            cv2.putText(img,f'No of Faces: {len(faces)}',(50,70),cv2.FONT_HERSHEY_COMPLEX,1,(255,0,0),2)
